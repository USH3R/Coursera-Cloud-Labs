...
aws ec2 run-instances \
  ...
  --user-data "file://$6" \
  --tag-specifications "ResourceType=instance,Tags=[{Key=module2-tag,Value=$7}]"'
  --tag-specifications 'ResourceType=instance,Tags=[{Key=module2-tag,Value=$7}]' \
  --output text \
...
INSTANCEIDS=$(aws ec2 describe-instances --output text --query 'Reservations[*].Instances[*].InstanceId' --filter "Name=instance-state-name,Values=pending,running")
